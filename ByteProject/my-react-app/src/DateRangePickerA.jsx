...
function DateRangePickerA() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      // Fetch plot data
      const plotResponse = await axios.post('http://localhost:5000/plotA', { start_date: startDate, end_date: endDate });
      setPlotData(plotResponse.data);

      // Fetch news data
      const newsResponse = await axios.post('http://localhost:5000/api/amzn', { start_date: startDate, end_date: endDate });
