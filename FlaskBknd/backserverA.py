...
def generate_plot(start_date, end_date):
    ...
    df = pd.read_csv('FlaskBknd/AMZN.csv')
    ...
    fig = px.line(df_range, x='Date', y='Average', title=f'AMZN Average of Open, High, and Low for {start_date} to {end_date}')
    fig.update_xaxes(title_text='Date', tickangle=45, tickformat="%Y-%m-%d")
    fig.update_yaxes(title_text='Average in $')
    
    return fig

@app.route('/plotamzn', methods=['POST'])
def plot():
